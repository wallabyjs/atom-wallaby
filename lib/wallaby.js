...
let updateCheckInterval = 1 * 60 * 1000;
let Q;
  ...
  activate(state) {
    ...
    setImmediate(() => {
      ...
      self._updateIfRequired(true).then(() => {
        ...
        state.load = done => {
          ...
          self._updateChecker = setInterval(() => self._updateIfRequired(false)
            .then(([newCore, newPlugin]) => {
              if (newCore) {
                self._newCore = newCore;
              }
              if (newPlugin) {
                self._newPlugin = newPlugin;
              }

              if (newCore || newPlugin) {
                state.installUpdate = ((coreZip, pluginZip) => () => {
                  let updates = [];
                  if (coreZip) updates.push(Wallaby._extract(coreZip, self._coreFolder));
                  if (pluginZip) updates.push(Wallaby._extract(pluginZip, self._pluginFolder));
        ...
        console.log('Wallaby Console: Wallaby.js Atom plugin v' + Wallaby._requireNoCache(self._pluginPackage).version);
        console.log('Wallaby Console: Wallaby.js Core v' + Wallaby._requireNoCache(self._corePackage).version);
      })
        .fail(e => self._notifyError(e.message))
