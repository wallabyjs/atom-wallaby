...
let phantomBinary = path.join(__dirname, '..', 'node_modules', 'phantomjs-prebuilt/lib/phantom/bin', 'phantomjs' + (isWin ? '.exe' : ''));
let oldlkp = path.join(path.join(__dirname, '..'), '..', '.wlck');
let lkp = path.join(os.homedir(), '.wallaby', 'key.lic');
...
class Wallaby {
  ...
  activate() {
    ...
    setImmediate(() => {
      ...
      try {
        phantomBinary = require('phantomjs-prebuilt').path || phantomBinary;
      }
      catch (e) {
      }

      self._isDebugMode = Wallaby._isDebugMode();
      self._noAutomaticInstallMode = Wallaby._noAutomaticInstallMode();

      self._baseUrl = 'https://update.wallabyjs.com/';
      self._versionFile = path.join(__dirname, '..', 'wallaby.json');
      self._updateLockFile = path.join(__dirname, '..', 'update.lock');
      self._updateLockOpts = {stale: updateCheckInterval};
      self._corePackage = path.join(__dirname, '..', self._isDebugMode ? '..' : '', 'wallaby', 'package.json');
      self._coreFolder = path.dirname(self._corePackage);
      self._pluginPackage = path.join(__dirname, '..', self._isDebugMode ? '..' : '', 'wallaby-atom', 'package.json');
      self._pluginFolder = path.dirname(self._pluginPackage);
      self._pluginIndex = path.join(self._pluginFolder, 'index.js');

      if (self._isDebugMode || self._noAutomaticInstallMode) {
        Wallaby._download = () => {
          throw new Error('Can not delete/download/extract in this mode');
        };
      }

      self._setupStatusIndicator();

      if (!fs.existsSync(lkp) && fs.existsSync(oldlkp)) {
        lkp = oldlkp;
      }

      self._updateIfRequired(true, undefined, self._expiryDate(lkp)).then(() => {
        let pluginState = {restore: global._wallabyPluginState};
        pluginState.updateAvailable = s => {
          s && s.onUpdateFinished && s.onUpdateFinished();
        };
  ...
  _performUpdate(pluginState) {
    let self = this;
    return self._updateIfRequired(false, pluginState.licensed, self._expiryDate(lkp))
          ...
          pluginState.updateAvailable(state => {
            try {
              ...
              Q.all(updates)
                ...
                .fail(e => self._notifyError(e.message))
                .then(() => state.onUpdateFinished && state.onUpdateFinished());
            }
            catch (e) {
              state.onUpdateFinished && state.onUpdateFinished();
            }
          });
        }
      })
  ...
  _updateIfRequired(fullInstall, isLicensed, expiryDate) {
    ...
    let downloadBaseUrl = self._baseUrl;

    delete global._wallabyHasJustUpdated;

    let downloadVersion = metadataBaseUrl => Wallaby._download({
      name: 'version',
      from: metadataBaseUrl + 'wallaby.json?licensed=' + (isLicensed && 'true' || 'false')
    ...
    return Q.all([
      ...
      .then(([coreStat, pluginStat]) => {
        return Q.all([coreStat, pluginStat,
          (self._isDebugMode || self._noAutomaticInstallMode || hasJustUpdated || (fullInstall && coreStat && pluginStat))
            ? Q.when({
              core: Wallaby._requireNoCache(self._corePackage).version,
              plugin: Wallaby._requireNoCache(self._pluginPackage).version
            })
            : (self._statusUpdater('Downloading wallaby.js version file'),
              downloadVersion(self._baseUrl)
                .fail(() => {
                  downloadBaseUrl = 'https://s3.amazonaws.com/wallaby-downloads/';
                  return downloadVersion('http://update.wallabyjs.com/');
                })
      ...
      .then(([coreStat, pluginStat, latestVersion]) => {
        ...
        if (lockFile.checkSync(self._updateLockFile, self._updateLockOpts)) {
        ...
        if (!(coreStat && (Wallaby._requireNoCache(self._corePackage).version === latestVersion.core))) {
          versions[0] = Wallaby._download({
            name: 'core',
            from: downloadBaseUrl + 'wallaby-v' + latestVersion.core + '.zip',
            to: self._coreFolder,
            unzip: fullInstall
          });
        ...
        if (!(pluginStat && (Wallaby._requireNoCache(self._pluginPackage).version === latestVersion.plugin))) {
          versions[1] = Wallaby._download({
            name: 'Atom plugin',
            from: downloadBaseUrl + 'wallaby-atom-v' + latestVersion.plugin + '.zip',
        ...
        if (!everythingIsUpToDate) {
          lockFile.lockSync(self._updateLockFile, self._updateLockOpts);
        }
        ...
        return Q.all(versions).then(v => {
          if (!everythingIsUpToDate) {
            lockFile.unlockSync(self._updateLockFile);
          }
          if (fullInstall && !everythingIsUpToDate) {
  ...
  static _noAutomaticInstallMode() {
    return process.env.wallabyNoAutomaticInstall;
  }
  ...
  static _extract(from, to) {
    return Wallaby._remove(to, true).then(() => {
      try {
        let Archive = require('adm-zip-mit');
  ...
  _expiryDate(lkp) {
    try {
      return new Buffer(require('fs').readFileSync(lkp).toString(), 'base64').toString().split('\n')[2];
    }
    catch (e) {
    }
  }
