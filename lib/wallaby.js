...
class Wallaby {
  ...
  activate(state) {
    ...
    // todo: ping
    // todo: install when restarting a session
    ...
    setImmediate(() => {
      ...
      Wallaby._updateIfRequired(true).then(() => {
        state = state || {};
        state.load = done => {
          ...
          self._updateChecker = setInterval(() => Wallaby._updateIfRequired(false)
            .then(([newCore, newPlugin]) => {
              if (newCore || newPlugin) {
                state.installUpdate = () => {
                  let updates = [];
                  atom.notifications.addInfo('Updating wallaby.js components');
        ...
        self._package = require(self._pluginFolder);
  ...
  static _updateIfRequired(fullInstall) {
    let self = this;
    ...
    return Q.all([
      ...
      .then(([coreStat, pluginStat, latestVersion]) => {
        ...
        self._coreVersion = Wallaby._requireNoCache(self._corePackage).version;
        self._pluginVersion = Wallaby._requireNoCache(self._pluginPackage).version;
